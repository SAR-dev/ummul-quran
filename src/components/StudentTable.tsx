...
const StudentTable = ({ students }: { students: StudentListType[] }) => {
    ...
    return (
        <div className="relative overflow-x-auto">
            <table className="w-full min-w-[40rem] text-sm text-left table-auto">
                ...
                <tbody>
                    {students.map((student, i) => (
                        <tr className="bg-base-100 border-b border-base-300 hover:bg-info/20 duration-200 cursor-pointer" onClick={() => navigate(`/students/${student.students_id}`)} key={i}>
                            <th scope="row" className="p-2 md:px-6 md:py-4">
                            ...
                            <td className="p-2 md:px-6 md:py-4">{student.user.location}</td>
                            <td className="p-2 md:px-6 md:py-4">{student.pack.minutes} Min</td>
                            <td className="p-2 md:px-6 md:py-4">{formatDate(student.last_class)}</td>
                            <td className="p-2 md:px-6 md:py-4">{formatDate(student.next_class)}4</td>
                        </tr>
                    ))}
                </tbody>
            </table>
